...
class CameraWorker(threading.Thread):
    ...
    def _fall_transition(self, history, feat, now):
        ...
        if len(slow_recent) >= 2:
            # Range (bounding-box diagonal) of the person's center position
            # over the window, NOT cumulative path length. Path length sums
            # every frame-to-frame delta, so it grows without bound under
            # ordinary per-frame pose-estimation noise (small keypoint
            # jitter, more pronounced outdoors / at lower resolution) even
            # when the person hasn't actually moved anywhere - this
            # previously made "stationary" nearly impossible to satisfy
            # under noisy real-world conditions. Range instead asks "how far
            # did the position actually wander", which stays small for a
            # genuinely still person (noise oscillates around one spot) and
            # only grows for real, sustained movement.
            cx_vals = [x["f"].get("center_x", 0.0) for x in slow_recent]
            cy_vals = [x["f"].get("center_y", 0.0) for x in slow_recent]
            slow_movement = float(np.hypot(
                max(cx_vals) - min(cx_vals),
                max(cy_vals) - min(cy_vals),
            ))
            ...
            if candidate_since is not None:
                ...
                if lying_now:
                    ...
                    if now - self.fall_stable_since[tid] >= confirm:
                        # Extra hard gate before confirming, for BOTH paths:
                        ...
                        # means this is NOT reported as a fall yet.
                        #
                        # This used to only apply to slow-fall candidates.
                        # It is now applied to fast-fall candidates too:
                        # fast_pass only required *some* window of continuous
                        # "lying_now", which a repetitive motion (e.g.
                        # sit-ups: torso angle dips below the lying threshold
                        # for a large fraction of every rep) could satisfy by
                        # chance without the person ever being still. A
                        # genuine fast fall is expected to end in real
                        # stillness well within this window, so this does not
                        # meaningfully delay real detections.
                        mode = self.fall_candidate_mode.get(tid)
                        slow_posture_threshold = float(
                            self.g.get("slow_fall_posture_threshold", 0.45)
                        )
                        slow_angle_limit_gate = float(
                            self.g.get("slow_fall_angle", 50.0)
                        )
                        slow_stationary_movement = float(
                            self.g.get("slow_fall_stationary_movement", 0.055)
                        )
                        # lying_score is aspect/spread-weighted, which
                        # assumes a roughly side-on camera where a
                        # horizontal body reads as a wide bbox. For a
                        # steep/overhead camera (e.g. an outdoor yard
                        # cam looking down) that assumption doesn't
                        # hold: a person lying flat directly under the
                        # camera never produces a wide bbox, so
                        # lying_score can stay artificially low even
                        # after a real fall. When a genuine torso angle
                        # measurement is available and shows the person
                        # is actually near-horizontal, that is
                        # independently sufficient - it does not depend
                        # on camera framing the way lying_score does.
                        posture_ok = feat["lying_score"] >= slow_posture_threshold or (
                            feat.get("angle_valid", True)
                            and feat["angle"] <= slow_angle_limit_gate
                        )
                        ...
                        if not gate_ok:
                            LOG.info(
                                "%s: %s-fall gate not yet met track=%s lying=%.2f "
                                "(need>=%.2f) angle=%.1f (need<=%.1f) move=%.3f (need<=%.3f)",
                                cid, mode or "?", tid, feat["lying_score"], slow_posture_threshold,
                                feat["angle"], slow_angle_limit_gate,
            ...
            if bool(self.g.get("unknown_onset_enabled", True)):
                ...
                if strongly_lying:
                    ...
                    if now - since >= unknown_seconds:
                        # This path exists specifically to catch someone who
                        # is genuinely down and immobile with no known prior
                        # standing state. Require actual stillness too -
                        # without it, sustained repetitive motion entirely on
                        # the floor (e.g. exercise) could satisfy "lying
                        # posture held a long time" without the person ever
                        # being still, which isn't the scenario this path is
                        # meant for. A genuinely incapacitated person is
                        # already immobile, so this does not cost recall for
                        # the real emergencies this path targets.
                        unknown_stationary_movement = float(
                            self.g.get("unknown_onset_stationary_movement")
                            if self.g.get("unknown_onset_stationary_movement") is not None
                            else self.g.get("slow_fall_stationary_movement", 0.055)
                        )
                        movement = details.get("slow_movement", 0.0)
                        if movement <= unknown_stationary_movement:
                            LOG.warning(
                            ...
                            LOG.info(
                                "%s: unknown-onset gate not yet met track=%s "
                                "duration=%.1fs move=%.3f (need<=%.3f)",
                                cid, tid, now - since, movement, unknown_stationary_movement,
                            )
                else:
